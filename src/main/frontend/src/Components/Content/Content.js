...
const Content = ({ title, description, btn, list }) => {
  return (
    <FlexBox flexJustify="center" column gap="30px">
      <Text fontSize="40px" fontWeight={600} lineHeight="70px" whiteSpace="pre">
        {title}
      </Text>
      <Text fontSize="16px" lineHeight="25px" whiteSpace="pre">
        {description}
      </Text>
          ...
          list.map(({ title, description, imgSrc }, index) => (
            <Box key={index}>
              ...
              <Box verticalMargin="15px">
                <Text fontSize="14px" fontWeight={700} whiteSpace="pre">
                  {title}
                </Text>
              </Box>
              <Text fontSize="13px" lineHeight="20px" whiteSpace="pre">
                {description}
              </Text>
            </Box>
          ))}
